...
for test_index in test_indices:
    ...
    sgd = torch.optim.SGD(model.parameters(), lr=ETA, weight_decay=0.001, momentum=0.80, nesterov=True)
    optimizer = adam # selected optimizer

    # learning rate scheduler
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.4, patience=100, verbose=True,min_lr=1e-5)
    ...
    # valid_loss_fn = FocalLoss(class_num=2, gamma=2.2, alpha=weight)
